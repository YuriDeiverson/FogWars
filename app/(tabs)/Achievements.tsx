...
import { ALL_ACHIEVEMENTS, RARITY_CONFIG, type Achievement } from '@/constants/achievements';
...
export default function AchievementsScreen() {
  const { unlockedAchievements, totalXP, level, getTileCount, getDistanceKm } = useGameStore();
    ...
    <View style={s.container}>
      ...
      <LinearGradient colors={['#050508', '#080c14']} style={s.header}>
        <View style={s.hTop}>
          ...
          <View style={s.hBadge}><Text style={s.hBadgeTxt}>{unlocked}/{total}</Text></View>
        </View>

        <View style={s.progWrap}>
          <View style={s.progTrack}>
            <View style={[s.progFill, { width: `${(unlocked / total) * 100}%` }]} />
          </View>
          <Text style={s.progTxt}>{((unlocked / total) * 100).toFixed(1)}% completo</Text>
        </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.filterBar} contentContainerStyle={s.filterContent}>
        {FILTERS.map((f) => (
          <TouchableOpacity key={f.id} style={[s.fBtn, filter === f.id && s.fBtnOn]} onPress={() => setFilter(f.id)}>
            <Ionicons name={f.icon} size={14} color={filter === f.id ? '#050508' : '#445566'} />
      ...
      <ScrollView contentContainerStyle={s.list} showsVerticalScrollIndicator={false}>
        {filtered.map((a) => (
          <AchCard key={a.id} ach={a} unlocked={unlockedAchievements.has(a.id)} />
        ))}
        {filtered.length === 0 && (
          <View style={s.empty}>
...
function AchCard({ ach, unlocked }: { ach: Achievement; unlocked: boolean }) {
  ...
  return (
    <View style={[c.card, !unlocked && c.locked]}>
      {unlocked && <View style={[c.accent, { backgroundColor: rarity.color }]} />}
      <View style={c.row}>
        <View style={[c.iconBox, { backgroundColor: unlocked ? rarity.color + '18' : '#111520', borderColor: unlocked ? rarity.color + '40' : '#1a2233' }]}>
          <Text style={[c.emoji, !unlocked && { opacity: 0.25 }]}>{unlocked ? ach.icon : '🔒'}</Text>
        </View>
        <View style={c.body}>
          <View style={c.titleRow}>
            <Text style={[c.title, !unlocked && { color: '#2a3a4a' }]}>{unlocked ? ach.title : '???'}</Text>
            <View style={[c.chip, { backgroundColor: unlocked ? rarity.color + '1a' : '#111520' }]}>
              <Text style={[c.chipTxt, { color: unlocked ? rarity.color : '#2a3a4a' }]}>{rarity.label}</Text>
            </View>
          </View>
          <Text style={[c.desc, !unlocked && { color: '#1e2a38' }]} numberOfLines={2}>
            {unlocked ? ach.description : 'Continue explorando para descobrir'}
          </Text>
          {unlocked && ach.curiosity && (
            <Text style={c.curiosity} numberOfLines={2}>💡 {ach.curiosity}</Text>
          )}
        </View>
        <View style={c.xpBox}>
          <Text style={[c.xpNum, !unlocked && { color: '#2a3a4a' }]}>{unlocked ? `+${ach.xp}` : ach.xp}</Text>
...
const s = StyleSheet.create({
  ...
  header:    { paddingTop: Platform.OS === 'ios' ? 56 : 40, paddingHorizontal: 18, paddingBottom: 18, borderBottomWidth: 1, borderBottomColor: '#1a2233' },
  ...
  hBadge:    { backgroundColor: 'rgba(0,255,150,0.12)', paddingHorizontal: 12, paddingVertical: 5, borderRadius: 20, borderWidth: 1, borderColor: 'rgba(0,255,150,0.25)' },
  hBadgeTxt: { color: '#00ff96', fontSize: 13, fontWeight: '700' },
  progWrap:  { marginBottom: 14, gap: 5 },
  progTrack: { height: 4, backgroundColor: '#1a2233', borderRadius: 2, overflow: 'hidden' },
  ...
  filterBar:    { maxHeight: 52, borderBottomWidth: 1, borderBottomColor: '#1a2233' },
  filterContent:{ flexDirection: 'row', paddingHorizontal: 14, gap: 8, alignItems: 'center', paddingVertical: 9 },
  fBtn:  { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20, backgroundColor: '#0c0e18', borderWidth: 1, borderColor: '#1a2233' },
...
const c = StyleSheet.create({
  card:    { backgroundColor: '#0c0e18', borderRadius: 16, borderWidth: 1, borderColor: '#1a2233', overflow: 'hidden' },
  locked:  { opacity: 0.5 },
  accent:  { height: 2 },
  ...
  curiosity:{ fontSize: 10, color: '#2a4a6a', fontStyle: 'italic', marginTop: 2, lineHeight: 14 },
